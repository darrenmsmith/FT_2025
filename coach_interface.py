...
def sessions():
    ...
    # Parse pattern_length from pattern_config for each session
    for session in sessions:
        if session.get('pattern_config'):
            try:
                pattern_config = json.loads(session['pattern_config'])
                session['pattern_length'] = pattern_config.get('pattern_length')
            except (json.JSONDecodeError, TypeError):
                session['pattern_length'] = None
        else:
            session['pattern_length'] = None
