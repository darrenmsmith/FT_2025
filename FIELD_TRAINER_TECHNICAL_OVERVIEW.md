...
Field Trainer supports multiple types of training drills, each with specialized implementations:

### Simon Says

**Purpose:** Reaction time and decision-making training drill

**Description:**
Simon Says is a cognitive agility drill where athletes respond to randomly activated field devices. The system controls which device(s) activate and in what pattern, requiring athletes to quickly identify and respond to the correct target.

**Operating Modes:**
- **Sequential Mode:** Devices activate one at a time in random order
- **Pattern Mode:** Custom patterns with multiple segments and per-athlete sequences
- **Multi-Athlete:** Each athlete can have independent patterns running simultaneously

**Technical Implementation:**
- Service: `/opt/services/session_service.py` (SimonSaysSession class)
- Routes: `/opt/routes/sessions_bp.py`
- Templates:
  - `/opt/field_trainer/templates/session_setup_cones.html` (setup)
  - `/opt/field_trainer/templates/session_monitor.html` (monitor)

**Database Tables:**
- `sessions` - Session metadata and configuration
- `runs` - Individual athlete run records
- `simon_says_patterns` - Pattern definitions
- `simon_says_segments` - Pattern segment configurations

**LED Patterns:**
- Active device: Solid color (configurable per segment)
- Success: Green flash
- Failure: Red flash

**Key Features:**
- Pattern-based segment design
- Per-athlete independent patterns
- Configurable segment duration and inter-segment delays
- Real-time pattern validation and status tracking
- Multi-athlete simultaneous execution

---

### Beep Test (Léger Protocol)

**Purpose:** Progressive cardiovascular fitness assessment

**Description:**
The Beep Test is a maximal multistage shuttle run test that progressively increases in difficulty. Athletes run between two points (15m or 20m apart) at speeds synchronized to audio beeps. The test has 21 levels, with speed increasing from 8.5 km/h (Level 1) to 18.5 km/h (Level 21).

**Technical Specifications:**
- **Levels:** 21 progressive stages
- **Speed Range:** 8.5 - 18.5 km/h (0.5 km/h increments)
- **Distance Options:** 15m or 20m
- **Shuttles per Level:** Varies from 7 (Level 1) to 16 (Level 21)
- **Duration:** ~22 minutes maximum (all 21 levels)

**Protocol:**
1. Athletes start at a designated line
2. **RUNNING Phase:** Athletes run to opposite line before beep sounds
3. **RECOVERY Phase:** Brief pause after completing shuttle (shuttle_time delay)
4. Speed increases each level
5. Athletes fail when they can't complete shuttle before beep twice consecutively

**Audio Patterns:**
- **1 Beep:** Start next shuttle (RUNNING phase begins)
- **2 Beeps:** Shuttle completed successfully (RECOVERY phase)
- **3 Beeps:** New level starting (inter-level transition)

**LED Patterns:**
- **RUNNING Phase:** `solid_green` - Constant green while running
- **RECOVERY Phase:** `chase_green` - Animated pattern during rest period

**Phases:**
1. **Setup:** Configure distance, start level, athlete order
2. **Deployment:** Deploy course to field devices
3. **Ready:** Athletes positioned, waiting for GO
4. **Active:** Test running with real-time monitoring
5. **Completed:** All athletes finished, results available

**Technical Implementation:**

**Service Layer:**
- File: `/opt/services/beep_test_service.py`
- Class: `BeepTestService`
- Thread: `BeepTestThread` (manages timing and state)
- Features:
  - Léger timing tables for all 21 levels
  - Phase tracking (running vs recovery)
...
**API Routes:**
- Blueprint: `/opt/routes/beep_test_bp.py`
- Endpoints:
  - `GET /beep-test/setup` - Setup page
  - `POST /api/beep-test/create-session` - Create new session
  - `POST /api/beep-test/deploy` - Deploy course to devices
  - `POST /api/beep-test/start` - Start the test
  - `GET /beep-test/monitor/<session_id>` - Monitor page
  - `GET /api/beep-test/status/<session_id>` - Real-time status
  - `GET /api/beep-test/athletes/<session_id>` - Athlete list
  - `POST /api/beep-test/toggle-athlete` - Fail/reactivate athlete
  - `POST /api/beep-test/stop` - End test early
  - `GET /beep-test/results/<session_id>` - Results page

**UI Components:**

*Setup Page:* `/opt/field_trainer/templates/beep_test_setup.html`
- Team selection
- Athlete drag-and-drop ordering (Sortable.js)
- Mark athletes absent
- Distance selection (15m/20m)
- Start level selection (1-21)
- Device count configuration
- Continue from last test option

*Monitor Page:* `/opt/field_trainer/templates/beep_test_monitor.html`
- Horizontal stats layout:
  - **LEVEL:** Large display (5rem font), current of 21
  - **Shuttle:** Current/max shuttles for level, current speed
  - **Time Between Beeps:** Max time and countdown (recovery phase only)
  - **Elapsed Time:** Total test duration
- Real-time athlete list:
  - Active/failed status badges
  - Fail level and shuttle information
  - Manual fail/reactivate controls
- GO button (when ready)
- END TEST EARLY button (when active)
- Auto-refresh every 1 second

*Results Page:* `/opt/field_trainer/templates/beep_test_results.html`
- Session summary
- Per-athlete results with highest level achieved
- Failed at level/shuttle information
...
*beep_test_sessions:*
```sql
CREATE TABLE beep_test_sessions (
    ...
    course_deployed INTEGER,  -- Boolean
    started_at TEXT,
    ended_at TEXT,
    final_level INTEGER,
...
*beep_test_results:*
```sql
CREATE TABLE beep_test_results (
...
- Beep test sessions also create entries in the main `sessions` table
- `drill_type = 'beep_test'`
- Links to team and course ("Beep Test - 20m" or "Beep Test - 15m")

**Dashboard Integration:**

*Recent Activity:*
- Beep test completions appear in chronological activity feed
- Display format: "Completed" badge (green) instead of time
- Shows course name and time ago

*Course Rankings:*
- Beep test courses ("Beep Test - 20m", "Beep Test - 15m") appear in rankings
- Ranking metric: Highest level achieved
- Display format: "Level X" badge instead of time
- Conditional rendering based on `ranking_type = 'beep_test'`

**Timing System:**

The Beep Test uses the official Léger Protocol timing tables. Example progression:

| Level | Speed (km/h) | Shuttles | Shuttle Time (sec) |
|-------|-------------|----------|-------------------|
| 1     | 8.5         | 7        | 8.47              |
| 5     | 10.5        | 9        | 6.86              |
| 10    | 13.0        | 11       | 5.54              |
...
**Countdown Timer:**
- Only displays during RECOVERY phase
- Counts down from shuttle_time to 0
- Hidden during RUNNING phase (shows "--")
- Updated every second via JavaScript
- Phase detection from `/api/beep-test/status` endpoint

**Deployment Flow:**
1. Create session and select athletes
2. Deploy course to field devices (creates START/END cones)
3. Monitor page shows READY status with GO button
4. Click GO to start test
5. Real-time monitoring with athlete status tracking
6. Automatic completion or manual early stop
7. Results page with athlete achievements

**Key Features:**
- Official Léger Protocol timing
- Phase-aware UI (running vs recovery)
- Real-time athlete monitoring
- Manual fail/reactivate controls
- LED visual feedback
- Audio beep patterns
- Dashboard integration with rankings
