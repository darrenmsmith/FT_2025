...
def connect_to_device0(node_id):
    ...
    audio_manager = AudioManager(audio_dir="/opt/field_trainer/audio/male")
    ...
    while True:
        try:
            ...
            while True:
                ...
                global last_touch_time
                current_time = time.time()
                print(f"DEBUG: Heartbeat - current_time={current_time:.3f}, last_touch_time={last_touch_time:.3f}, diff={(current_time - last_touch_time):.3f}")
                if last_touch_time > 0 and (current_time - last_touch_time) < 5.0:
                    # Touch occurred within last second, report it
