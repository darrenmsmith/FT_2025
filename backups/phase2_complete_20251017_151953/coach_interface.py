...
def find_athlete_for_touch(device_id: str, timestamp: datetime) -> Optional[str]:
    ...
    # Find athletes who should be at this device
    candidates = []
    ...
    for run_id, run_info in active_session_state['active_runs'].items():
        ...
        print(f"      {run_info['athlete_name']}: last_position={last_position}, needs={device_position-1}")

        # Check if this athlete should be at this device next
        if last_position == device_position - 1:
            # Check if they've touched this device already
            # For first device, check sequence_position
            # For other devices, check segments
            if device_position == 0:
                # First device - check if sequence_position >= 0
                already_touched = (last_position >= 0)
            else:
                # Other devices - check segments
                segments = db.get_run_segments(run_id)
                already_touched = any(
                    seg.get('to_device') == device_id and seg.get('touch_detected')
                    for seg in segments
                )
            
            if not already_touched:
                candidates.append((run_id, run_info['started_at']))
                print(f"   ✅ Candidate: {run_info['athlete_name']} (started {run_info['started_at']})")
    
    if not candidates:
        print(f"   ⚠️  No valid candidates for device {device_id}")
        return None
    
    # Return earliest started athlete
    candidates.sort(key=lambda x: x[1])
    selected_run_id = candidates[0][0]
    print(f"   🎯 Selected: {active_session_state['active_runs'][selected_run_id]['athlete_name']}")
