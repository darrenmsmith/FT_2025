...
def session_monitor(session_id):
    ...
    # Use beep test monitor template for beep test courses
    if course.get('course_type') == 'beep_test':
        return render_template(
            'beep_test_monitor.html',
            session=session,
            team=team,
            course=course
        )
...
def deploy_course(session_id):
    ...
    try:
        ...
        # For Beep Test courses, store beep test configuration
        if course.get('mode') == 'group':
            from flask import request
            import json
            ...
            if beep_config:
                print(f"   🏃 Beep Test config specified:")
                print(f"      Distance: {beep_config.get('distance_meters')}m")
                print(f"      Device Count: {beep_config.get('device_count')}")
                print(f"      Start Level: {beep_config.get('start_level')}")

                # Store beep test config in session (reuse pattern_config field)
                with db.get_connection() as conn:
                    conn.execute('''
                        UPDATE sessions
                        SET pattern_config = ?
                ...
                # Initialize beep test results for all athletes on the team
                print(f"   👥 Initializing beep test results for team athletes...")

                # Get all athletes on the team
                athletes = db.get_athletes_by_team(session['team_id'])

                # Create beep_test_results entry for each athlete
                for athlete in athletes:
                    try:
                        db.add_athlete_to_beep_test(
                            session_id=session_id,
                            athlete_id=athlete['athlete_id']
                        )
                        print(f"      ✅ Added {athlete['name']} to beep test")
                    except Exception as e:
                        print(f"      ⚠️  Failed to add {athlete['name']}: {e}")

                print(f"   ✅ {len(athletes)} athletes initialized for beep test")
